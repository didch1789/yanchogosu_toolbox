function [cverr, stats, optout] = predict_temp(obj, varargin)
...
% intercept = b(1,:);
% 
% yfit = [ones(size(xtest,1),1) xtest]*b;
% 
% end
...
wh = find(strcmp(varargin, 'numcomponents'), 1);
...
[~, ~, ~, ~, beta] = pls_ycgosu(xtrain, ytrain, numc);

vox_weights = beta(2:end, :);

intercept = beta(1, :);

yfit = [ones(size(xtest, 1), 1) xtest] * beta; 
