...
%
    ...
    for i = 1:length(varargin)
        if ischar(varargin{i})
            switch varargin{i}
                ...
                case 'out_name', out_name = varargin{i+1};
                    
                case 'doweights', doweights = 1;
    ...
    if doamps
        ...
        imgs = dir('beta*nii'); imgs = char(imgs.name);
        n = size(imgs, 1);
            ...
            for i = 1: nbf : (n - nbf + 1)
                ...
                switch derivative_case
                    ...
                    case 'timedispersion'
                        ...
                        spm_imcalc_ui(imgs_cond, out_name,...
                            ['sign(i1) .* sqrt(i1.^2 .*', num2str(weights(1)), '+ i2.^2 .*', num2str(weights(2)), ...
                            '+ i3.^2.*', num2str(weights(3)), ')']);
                    otherwise
        ...
        if nohtw
            ...
            if exist('startend', 'var')
                ...
                % Set range in sec
                htw_from_fit(SPM.xBF.bf, ones(size(SPM.xBF.bf, 2), 1), mytimeres);
            ...
            for i = 1: nbf : (n - nbf + 1)
                ...
                output_name{4} = sprintf('htw_area_under_curve_%s.img', nameof);

                % ---------------------------------------------
                [h, t, w, auc] = image_eval_function(imgs_cond, htwfunction, 'mask', mask, ...
                  'outimagelabels', output_name);
