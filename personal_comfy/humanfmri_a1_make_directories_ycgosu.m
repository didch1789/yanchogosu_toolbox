function PREPROC = humanfmri_a1_make_directories_ycgosu(subject_code, study_imaging_dir, func_run_nums, func_tasks)

% The function creates directories for dicom files
%
% :Usage:
% ::
%    subject_dir = humanfmri_a1_make_directories(subject_code, study_imaging_dir, func_run_nums, func_tasks)
%
% :Inputs:
% ::
% 
...
if ~iscell(subject_code)
    ...
    tasks_cell{1} = func_tasks;
    func_run_nums_cell{1} = func_run_nums;
else
    subject_codes = subject_code;
    if ischar(func_tasks) % if tasks is char
        for subj_i = 1:numel(subject_codes)
            tasks_cell{subj_i} = func_tasks; 
        end
    elseif numel(subject_codes) ~= numel(func_tasks) % tasks is cell, but multiple tasks
        for subj_i = 1:numel(subject_codes)
            tasks_cell{subj_i} = func_tasks; 
        end
    else % if tasks is cell and each cell for each subject
        tasks_cell = func_tasks;
    end
    ...
    if ~iscell(func_run_nums) % if func_run_nums is not in the cell
        ...
        func_run_nums_cell = func_run_nums;
    end
end
...
for subj_i = 1:numel(subject_codes)
    ...
    % func directory
    j = 2;
    for i = 1:numel(func_run_nums{subj_i})
        j = j + 1;
