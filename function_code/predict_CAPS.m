function [cverr, stats, optout] = predict_CAPS(obj, varargin)
...
%
%   **rolling_CAPS** = [h,v,g,sets,method]
%        same as rolling. just for CAPS2 (2 sets are used)
%        method : 1 is mixture(ccont-caps-cont-caps...), 2 is
%        concat(cont-cont-----caps-caps)
%
%   **savecv**
%        Save CV weight
%
    ...
    case 'cv_svm'
        ...
        if nfolds ~= 1
            ...
            for i = 1:length(teIdx)
                dd(teIdx{i},:) = cv_optout{i, 2}; %added by lc 6/10/13 to accomodate multiclass
            end
...
function [yfit, b, intercept] = cv_multregress_CAPS(xtrain, ytrain, xtest, cv_assignment, varargin)

X = [ones(size(xtrain, 1), 1), xtrain];
if rank(X) <= size(xtrain, 2)
...
if any(strcmp(varargin, 'onlyboot'))
    intercept = []; yfit = []; return
end
...
if ~doSkip
    ...
    % 7/27/17: J.J.Lee: revised lasso_rocha contents(*diag -> .*).
    if dopcr
        ...
        else out = lasso_rocha_CAPS(ytrain, sc); end
    end
...
function [yfit, vox_weights, intercept, out_cv] = cv_lassopcr_CAPS(xtrain, ytrain, xtest, cv_assignment, varargin) % 3/23/13 Wani Woo added out_cv.
...
if ~doSkip
    ...
    if dopcr
        if doNestedXval, out = lasso_cv(ytrain, sc(:, 1:numcomps), length(unique(cv_assignment)), options);
        else out = lasso_rocha_CAPS(ytrain, sc(:, 1:numcomps)); end
    else
        if doNestedXval, out = lasso_cv(ytrain, sc, length(unique(cv_assignment)), options);
        else out = lasso_rocha_CAPS(ytrain, sc); end
    end
    ...
    n = find(n_in_model == lassonum, 1, 'last'); % find the lambda of least shrinkage
...
function [yfit, vox_weights, intercept] = cv_cpm(xtrain, ytrain, xtest, cv_assignment, varargin)
...
issigedge = true;
if cpm_pos
    pos_edges = find(r_mat > 0 & p_mat < cpm_thresh);
    if numel(pos_edges) == 0; issigedge = false; end
    wh_pos = r_mat > 0 & p_mat < cpm_thresh;
end
if cpm_neg
    neg_edges = find(r_mat < 0 & p_mat < cpm_thresh);
    if numel(neg_edges) == 0; issigedge = false; end
    wh_neg = r_mat < 0 & p_mat < cpm_thresh;
end

if issigedge
    
    if cpm_pos; fprintf('%.8d/%.8d positive weight edges were detected.\n', numel(pos_edges), no_node); end
    if cpm_neg; fprintf('%.8d/%.8d negative weight edges were detected.\n', numel(neg_edges), no_node); end
...
%     end
    ...
    error('no significant weight edges were detected. it`ll get an error.')
    yfit = [];
