...
if ~isempty(wh) && length(varargin) >= wh + 1
    ...
    numc = varargin{wh + 1};
    if ischar(numc)
        numExplVar = str2double(numc);
        fprintf('Extracting components that explain %02d perc of the variance\n', numExplVar);
        [~, ~, ~, ~, expls] = pca(xtrain);warning off;
        numc = max(find(cumsum(expls) < numExplVar));
    end
    ...
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case 'numcomponents'
                ...
                numc = varargin{i + 1};
                if isempty(numc)
                    error('Follow numcomponents or percentage with number of components to keep');
                    varargin{i+1} = [];
                    chooseComponents = 1;
                elseif ischar(numc)
                    doextractPercentage = true;
                end
            case 'lasso_num'
                varargin{i} = [];
